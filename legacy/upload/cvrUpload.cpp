...
rtConnection CVRUpload::connectionSend;
...
void CVRUpload::freeResources()
{
	...
	{
		RDK_LOG( RDK_LOG_DEBUG,"LOG.RDK.CVRUPLOAD","%s(%d): Remove file [%s] from camera ram.\n", __FILE__, __LINE__, upload_file_name);
...
void CVRUpload::notify_smt_TN(CVR_UPLOAD_STATUS status, char* upload_fname)
{
	...
	while (!term_flag)
	{
		...
		char modelName[128];
			...
			modelName[stdata.bufLen] = '\0';
                        RDK_LOG(RDK_LOG_DEBUG,"LOG.RDK.CVRUPLOAD","%s(%d): Model Name = %s\n", __FILE__, __LINE__,modelName);
                ...
                {
                        RDK_LOG(RDK_LOG_ERROR,"LOG.RDK.CVRUPLOAD","%s(%d): GET ModelName failed.\n", __FILE__, __LINE__);
		...
		//only for testing
		/*RDK_LOG( RDK_LOG_INFO,"LOG.RDK.CVRUPLOAD","%s(%d) Before Calling notify_smt_TN.\n",__FILE__, __LINE__);
		notify_smt_TN(CVR_UPLOAD_OK, p_file_name);
		RDK_LOG( RDK_LOG_INFO,"LOG.RDK.CVRUPLOAD","%s(%d) After notify_smt_TN.\n",__FILE__, __LINE__);*/
		...
		if (CVR_UPLOAD_TIMEOUT == ret)
		{
			...
			notify_smt_TN(CVR_UPLOAD_TIMEOUT, p_file_name);
			break;
		}
		else if (CVR_UPLOAD_FAIL == ret)
		{
			...
			if (waitingInterval > ((CVR_UPLOAD_TIMEOUT_INTERVAL + start_upload_time ) - current_time))
			{
				...
				if ( ( 6 == response_code )  || (51 == response_code) || (60 == response_code) ) {
					notify_smt_TN(CVR_UPLOAD_CURL_ERR, p_file_name);
