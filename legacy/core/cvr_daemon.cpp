 ...
 */
int CVR::cvr_init(char *argv,CloudRecorderConf *pCloudRecorderInfo)
{
	int rdkc_ret = 1;
	...
	smartTnEnabled = check_enabled_rfc_feature(RFC_SMART_TN_UPLOAD, SMART_TN_UPLOAD);
	if(smartTnEnabled) {
	...
	//Check if smart thumbnail is enabled via RFC
	//smartTnEnabled = check_enabled_rfc_feature(RFC_SMART_TN_UPLOAD, SMART_TN_UPLOAD);
	...
	int streamNotreadyErrorCount = 0; // to restrict the logging of Stream Init Error Count 
		...
		else
		{
			...
			if(cvr_flag & RDKC_STRAM_FLAG_AUDIO)
				...
				{
					RDK_LOG( RDK_LOG_DEBUG,"LOG.RDK.CVR","(%d): rInitialize clip creation success!\n", __LINE__);
				}
                        }
                        else
                        {
...
int main(int argc, char *argv[])
{
	...
	int ret = cvr_object.cvr_init(argv[0],&CloudRecorderInfo);
...
error_exit:
	cvr_object.cvr_close(argv[0]);
        RDK_LOG( RDK_LOG_INFO,"LOG.RDK.CVR","%s(%d): %s exit! - sigterm flag - %d \n", __FILE__, __LINE__, *argv,CVR::term_flag);
	return 0;
}
