...
int CVR::cvr_init(int argc, char **argv,CloudRecorderConf *pCloudRecorderInfo)
{
	...
	RDK_LOG( RDK_LOG_DEBUG,"LOG.RDK.CVR","%s(%d): amba_hwtimer_init success\n", __FILE__, __LINE__);
        ...
        kvsclip_audio = 0;/* audio enable flag */
					...
					bool isEOF = false)
{
    ...
    if(clipStatus == CVR_CLIP_GEN_START)
    {
        if(kvsclip_audio)
        {
            RDK_LOG( RDK_LOG_INFO,"LOG.RDK.CVR","%s(%d): kvs clip has audio\n", __FILE__, __LINE__);
        }
    }
    ...
    if ( false == iskvsStreamInitDone )
    {
        ...
        ret_kvs = kvsStreamInit(kvsclip_audio, kvsclip_abstime, kvsclip_livemode, contentchangestatus);
        if ( true == ret_kvs )
        {
    ...
    do
    {
        RDK_LOG(RDK_LOG_DEBUG, "LOG.RDK.CVR","%s(%d): check values : %u %u %u\n", __FILE__, __LINE__, kvsclip_audio, kvsclip_abstime,kvsclip_livemode);

        ret_kvs = kvsUploadFrames(kvsclip_audio, kvsclip_abstime, kvsclip_livemode, frameInfo, fileName, isEOF);
        ...
        {
            RDK_LOG(RDK_LOG_INFO, "LOG.RDK.CVR", "%s(%d):  content change found for clip reinit kvs stream %s\n",__FILE__, __LINE__, fileName);
            iskvsStreamInitDone = false;
    ...
    while (!term_flag)
    {
            ...
            while (!term_flag)
            {
                ...
                else
                {
                    ...
                    if(IAV_PIC_TYPE_IDR_FRAME == cvr_frame.pic_type || IAV_PIC_TYPE_I_FRAME == cvr_frame.pic_type)
                        RDK_LOG( RDK_LOG_INFO, "LOG.RDK.CVR","Got the first I Frame \n");
                    else
                        RDK_LOG( RDK_LOG_INFO, "LOG.RDK.CVR","Frame obtained is not an I Frame \n");
        ...
        if (has_an_iframe)
        {
            ...
            if(cvr_flag & RDKC_STREAM_FLAG_AUDIO)
            {
                ...
                pushFrames(cvr_key_frame, file_name, ( m_streamid & 0x0F ), kvsclip_audio, kvsclip_abstime, kvsclip_livemode);
            }
            else
            {
        ...
        while ((long)(file_len*1000 - compare_timestamp(amba_hwtimer_msec(hwtimer_fd), start_msec)) > 20)
        {
            ...
            if(cvr_flag & RDKC_STREAM_FLAG_VIDEO )
            {
                ...
                else if (0 == ccode)
                {
                    ...
                    if (!has_an_iframe)
                    {
                        pushFrames(cvr_frame, file_name, ( m_streamid & 0x0F ), kvsclip_audio, kvsclip_abstime, kvsclip_livemode);
        ...
        if (!term_flag)
        {
            ...
            else
            {
                ...
                if(smartTnEnabled)
                {
                    ...
                    pushFrames(cvr_frame, file_name, ( m_streamid & 0x0F ), kvsclip_audio, kvsclip_abstime, kvsclip_livemode, event_type, true);
                }
                else
                {
