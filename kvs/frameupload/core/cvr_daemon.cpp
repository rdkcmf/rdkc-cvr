...
#if defined ( ENABLE_CVR_WITH_PIPEWIRE )
    if(pthread_mutex_init(&pip_cvrlock, NULL) != 0)
    {
        printf("%s(%d): Mutex init for cvr failed...\n", __FILE__, __LINE__);
        return -1;
    }

    while(1)
    {
        if ( pthread_mutex_lock( &pip_cvrlock ) != 0 )
        {
            return RDKC_FAILURE;
        }

        PIP_CVR_FrameInfo frame_info = { 0 };
        int pushframestatus = 0;

        int retVal = PIP_CVR_ReadFrameData ( &frame_info );
...
#if defined ( ENABLE_CVR_WITH_PIPEWIRE )
/* {{{ CVR_Mutex_unlock() */
void CVR_Mutex_unlock()
{
       pthread_mutex_unlock( &pip_cvrlock );
}
#endif /* ENABLE_CVR_WITH_PIPEWIRE */
