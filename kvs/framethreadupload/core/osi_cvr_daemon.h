...
#define KVSUPLOAD_Q_SIZE_LIMIT 1120
...
typedef struct kvsUploadData
{
    // frameInfoH264 *frames;
    s16 stream_id;                   // -1~3, -1 means read all stream frames
    u16 stream_type;                 // refer to enum iav_stream_type
    u32 pic_type;                    // refer to enum iav_pic_type
    string frame_ptr;                // the frame buffer pointer
    u32 frame_num;
    u32 frame_size;
    u32 jpeg_quality;
    u32 width;
...
class CVR : public kvsUploadCallback
{
    private:
      ...
      static unsigned long lastUploadedFrame_ts; // Frame timestamp, in milliseconds
      std::queue<kvsUploadData_t> kvsDataQ;
      std::mutex kvsFramePushMutex;
      std::mutex kvsFrameCVRMutex;
      std::condition_variable msgCv_kvs;
      bool cvrDumpEnabled;
      /* rtmessage */
#ifdef RTMSG
      ...
      int m_streamdump;
      int m_skipAudioFrameCount;
      bool m_kvsUploadThreadExit;
      ...
      void push_KvsFrameInfo(kvsUploadData_t kvsFrameInfo);
      void pop_KvsFrameInfo(kvsUploadData_t *kvsFrameInfo);
      void pushFramesToKVS();
    public:
      CVR();
      ~CVR();
